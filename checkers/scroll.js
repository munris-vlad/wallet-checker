...
const marksApi = "https://mainnet-api-sessions.scroll.io/v1/session2-marks/"
...
async function getTxs(wallet, index) {
    ...
    while (!isMarksCollected) {
        await axios.get(marksApi+wallet, {
            ...
            marks = response.data.result.marks ? parseFloat(response.data.result.marks).toFixed(2) : 0
