                ...
                <tr v-for="(item, index) in sortedData" :key="index" class="border-b dark:border-gray-700">
                    ...
                    <td :class="tdClass + ' text-left'">
                        <div class="flex space-x-2 pt-3 pb-2">
                            ...
                            <div class="h-4 w-4" v-if="item['wallet'] !== 'Total'">
                                <a target="_blank" :href="'https://explorer.morphl2.io/address/'+item['wallet']"><img class="rounded-full mb-1" :src="'/morph-scan.png'" alt=""></a>
                            </div>
                        </div>
                    </td>
                    <td :class="[tdClass, parseFloat(item['ETH']) < appconfig.modules.morph.minBalanceHighlight ? 'text-red-500' : '']">{{ item['ETH'] }} (${{ item['ETH USDVALUE'] }})</td>
...
export default {
    ...
    methods: {
        formatDate,
        loadData() {
            this.$axios.get('/api/morph').then((response) => {
        ...
        fetchWallet(wallet, index) {
            ...
            this.$axios.get('/api/morph/refresh', {params: {wallet: wallet}}).then(() => {
                this.loadData()
        ...
        refreshData() {
            ...
            this.$axios.get('/api/morph/clean').then(() => {
                this.loadData()
            }).catch((error) => {
                this.isError = true
                this.error = error.toString()
