                    ...
                    <td :class="tdClass + ' text-left'">
                        <div class="flex space-x-2 pt-3 pb-2">
                            ...
                            <div class="h-4 w-4" v-if="item['wallet'] !== 'Total'">
                                <a target="_blank" :href="'https://explorer.solana.com/address/'+item['wallet']"><img class="rounded-full mb-1" :src="'/solana-scan.png'" alt=""></a>
                            </div>
                        </div>
                    </td>
                    <td :class="[tdClass, parseFloat(item['SOL']) < appconfig.modules.solana.minBalanceHighlight ? 'text-red-500' : '']">{{ item['SOL'] }} (${{ item['SOL USDVALUE'] }})</td>
                    <td :class="tdClass">
...
export default {
    ...
    methods: {
        formatDate,
        loadData() {
            this.$axios.get('/api/solana').then((response) => {
        ...
        fetchWallet(wallet, index) {
            ...
            this.$axios.get('/api/solana/refresh', {params: {wallet: wallet}}).then(() => {
                this.loadData()
        ...
        refreshData() {
            ...
            this.$axios.get('/api/solana/clean').then(() => {
                this.loadData()
            }).catch((error) => {
                this.isError = true
                this.error = error.toString()
