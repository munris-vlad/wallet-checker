                ...
                <tr v-for="(item, index) in sortedData" :key="index" class="border-b dark:border-gray-700">
                    ...
                    <td :class="tdClass + ' text-left'">
                        <div class="flex space-x-2 pt-3 pb-2">
                            ...
                            <div class="h-4 w-4" v-if="item['Wallet'] !== 'Total'">
                                <a target="_blank" :href="`https://app.debridge.finance/statistic/${item['Wallet']}`"><img class="rounded-full mb-1" :src="'/debridge.png'" alt=""></a>
                            </div>
                        </div>
                    </td>
                    ...
                    <td :class="tdClass">{{ item['Multiplier'] }}</td>
                    <td :class="tdClass">
...
export default {
    ...
    methods: {
        formatDate,
        loadData() {
            this.$axios.get('/api/debridge').then((response) => {
        ...
        fetchWallet(wallet, index) {
            ...
            this.$axios.get('/api/debridge/refresh', {params: {wallet: wallet}}).then(() => {
                this.loadData()
        ...
        refreshData() {
            ...
            this.$axios.get('/api/debridge/clean').then(() => {
                this.loadData()
            }).catch((error) => {
                this.isError = true
                this.error = error.toString()
