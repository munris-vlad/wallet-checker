                    ...
                    <td :class="tdClass + ' text-left'">
                        <div class="flex space-x-2 pt-3 pb-2">
                            ...
                            <div class="h-4 w-4" v-if="item['wallet'] !== 'Total'">
                                <a target="_blank" :href="'https://eclipsescan.xyz/account/'+item['wallet']"><img class="rounded-full mb-1" :src="'/eclipse-scan.png'" alt=""></a>
                            </div>
                        </div>
                    </td>
...
export default {
    ...
    methods: {
        formatDate,
        loadData() {
            this.$axios.get('/api/eclipse').then((response) => {
        ...
        fetchWallet(wallet, index) {
            ...
            this.$axios.get('/api/eclipse/refresh', {params: {wallet: wallet}}).then(() => {
                this.loadData()
        ...
        refreshData() {
            ...
            this.$axios.get('/api/eclipse/clean').then(() => {
                this.loadData()
            }).catch((error) => {
                this.isError = true
                this.error = error.toString()
