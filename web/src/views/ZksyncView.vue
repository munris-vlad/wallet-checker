        ...
        <table class="min-w-full border text-center text-sm font-light dark:border-gray-700" v-if="isDataLoaded && !isError">
            ...
            <tbody>
                <tr v-for="(item, index) in sortedData" :key="index" class="border-b dark:border-gray-700">
                    ...
                    <td :class="[tdClass, parseFloat(item['ETH']) < appconfig.modules.zksync.minBalanceHighlight ? 'text-red-500' : '']">{{ item['ETH'] }} (${{ item['ETH USDVALUE'] }})</td>
...
export default {
    ...
    methods: {
        ...
        fetchWallet(wallet, index) {
            ...
            this.$axios.get('/api/zksync/refresh', {params: {wallet: wallet}}).then(() => {
                this.loadData()
        ...
        refreshData() {
            ...
            this.$axios.get('/api/zksync/clean').then(() => {
                this.loadData()
            }).catch((error) => {
                this.isError = true
                this.error = error.toString()
