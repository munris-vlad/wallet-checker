                ...
                <tr v-for="(item, index) in sortedData" :key="index" class="border-b dark:border-gray-700">
                    ...
                    <td :class="tdClass + ' text-left'">
                        <div class="flex space-x-2 pt-3 pb-2">
                            ...
                            <div class="h-4 w-4" v-if="item['wallet'] !== 'Total'">
                                <a target="_blank" :href="'https://testnet.monadexplorer.com/address/'+item['wallet']"><img class="rounded-full mb-1" :src="'/monad.png'" alt=""></a>
                            </div>
                        </div>
                    </td><td :class="[tdClass, parseFloat(item['MON']) < appconfig.modules.monad.minBalanceHighlight ? 'text-red-500' : '']">{{ item['MON'] }}</td>
...
export default {
    ...
    methods: {
        formatDate,
        loadData() {
            this.$axios.get('/api/monad').then((response) => {
        ...
        fetchWallet(wallet, index) {
            ...
            this.$axios.get('/api/monad/refresh', {params: {wallet: wallet}}).then(() => {
                this.loadData()
        ...
        refreshData() {
            ...
            this.$axios.get('/api/monad/clean').then(() => {
                this.loadData()
            }).catch((error) => {
                this.isError = true
                this.error = error.toString()
