                ...
                <tr v-for="(item, index) in sortedData" :key="index" class="border-b dark:border-gray-700">
                    ...
                    <td :class="tdClass + ' text-left'">
                        <div class="flex space-x-2 pt-3 pb-2">
                            ...
                            <div class="h-4 w-4" v-if="item['wallet'] !== 'Total'">
                                <a target="_blank" :href="'https://scan.li.fi/wallet/'+item['Wallet']"><img class="rounded-full mb-1" :src="'/jumper.svg'" alt=""></a>
                            </div>
                        </div>
                    </td>
                    ...
                    <td :class="tdClass + ' text-left'">
                        <div class="flex space-x-2 pt-3 pb-4 select-none max-w-sm">
                            <div class="h-4 w-4 text-center" v-for="(count, source) in item['sources']" :key="source" :title="names[source]">
                                <img class="rounded-full mb-1 h-4 w-4" :src="chains[source]" :alt="names[source]">
                    ...
                    <td :class="tdClass + ' text-left'">
                        ...
                        <div class="flex space-x-2 pt-3 pb-4 select-none max-w-sm">
                            <div class="h-4 w-4 text-center" v-for="(count, protocol) in item['protocols']" :key="protocol" :title="protocol">
                                <img class="rounded-full mb-1 h-4 w-4" :src="`/${protocol}.png`" :alt="protocol">
                                <span class="text-xs protocol-text">{{ count }}</span>
                            </div>
                        </div>
                    </td>
                    ...
                    <td :class="tdClass + ' text-left'">
                        <div class="flex space-x-2 pt-3 pb-4 select-none max-w-sm">
                            <div class="h-4 w-4 text-center" v-for="(count, destination) in item['destinations']" :key="destination" :title="names[destination]">
                                <img class="rounded-full mb-1 h-4 w-4" :src="chains[destination]" :alt="names[destination]">
                                <span class="text-xs protocol-text">{{ count }}</span>
                            </div>
                        </div>
                    </td>
...
export default {
    ...
    methods: {
        formatDate,
        loadData() {
            this.$axios.get('/api/jumper').then((response) => {
        ...
        fetchWallet(wallet, index) {
            ...
            this.$axios.get('/api/jumper/refresh', {params: {wallet: wallet}}).then(() => {
                this.loadData()
        ...
        refreshData() {
            ...
            this.$axios.get('/api/jumper/clean').then(() => {
                this.loadData()
            }).catch((error) => {
                this.isError = true
                this.error = error.toString()
