<template>
    <div class="pb-5">
        <div class="min-w-full text-center header pb-4 pt-4">
            <h1 class="text-3xl">Polymarket</h1>
        </div>
        ...
        <table class="min-w-full border text-center text-sm font-light dark:border-gray-700" v-if="isDataLoaded && !isError">
            ...
            <tbody>
                <tr v-for="(item, index) in sortedData" :key="index" class="border-b dark:border-gray-700">
                    ...
                    <td :class="tdClass">${{ item['Positions Value'] }}</td>
                    <td :class="tdClass">${{ item['PnL'] }}</td>
                    <td :class="tdClass">${{ item['Volume'] }}</td>
                    <td :class="tdClass">{{ item['Markets'] }}</td>
                    <td :class="tdClass">
...
export default {
    ...
    methods: {
        formatDate,
        loadData() {
            this.$axios.get('/api/polymarket').then((response) => {
        ...
        fetchWallet(wallet, index) {
            ...
            this.$axios.get('/api/polymarket/refresh', {params: {wallet: wallet}}).then(() => {
                this.loadData()
        ...
        refreshData() {
            ...
            this.$axios.get('/api/polymarket/clean').then(() => {
                this.loadData()
            }).catch((error) => {
                this.isError = true
                this.error = error.toString()
